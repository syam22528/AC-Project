...
from CHAM.cham_naive import encrypt as naive_enc
from CHAM.cham_optimized import encrypt as opt_enc
from CHAM.cham_numba import encrypt as numba_enc
from CHAM.cham_gpu import encrypt as gpu_enc

from CHAM.cham_optimized import key_schedule
from CHAM.cham_gpu import cham_encrypt_kernel



# -----------------------------
# Helper: bytes → uint32 blocks
# -----------------------------
def bytes_to_blocks(data):
    return np.frombuffer(data, dtype=np.uint32).reshape(-1, 4).copy()
...
def gpu_benchmark_detailed(data, key):
    ...
    cham_encrypt_kernel[blocks_per_grid, threads_per_block](d_blocks, d_rk)
